...
<div class="container mt-4" style="width: 30em; border: 2px solid #999; border-radius: 5px">
    ...
    <form action="/servletPaymentsApp/payment/getToCardConfPaymentForm" method="GET">
        <input type="hidden" name="senderMoneyAccId" value="${requestScope.senderMoneyAccId}">
        <label for="cardNum"><fmt:message key="payment.recCardNum"/></label>
        <input type="text" name="receiverCardNum" id="cardNum" class="form-control
        ${requestScope.errors.get('cardNumErrors') != null ? 'is-invalid' : 'is-valid'}"
        value="${requestScope.receiverCardNumber}">
        <c:if test="${requestScope.errors.get('cardNumErrors') != null}">
            <div class="alert alert-danger" role="alert">
                <c:forEach var="error" items="${requestScope.errors.get('cardNumErrors')}">
                    ${error}<br>
                </c:forEach>
            </div>
        </c:if>
